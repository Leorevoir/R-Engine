...
static auto resolve_parent_entity(const RenderData& data, r::ecs::Entity child, r::ecs::Entity parent) -> r::ecs::Entity {
    if (parent != PLACEHOLDER) return parent;
    auto it = data.parent_from_children.find(child);
    if (it != data.parent_from_children.end()) return it->second;
...
static auto resolve_parent_for_foreground(const RenderData& data, r::ecs::Entity child, r::ecs::Entity parent) -> r::ecs::Entity {
    if (parent < PLACEHOLDER_THRESHOLD) return parent;
    auto parent_it = data.parent_from_children.find(child);
    if (parent_it != data.parent_from_children.end()) return parent_it->second;
    return r::ecs::NULL_ENTITY;
}
...
static auto collect_draw_items(QueryType& q) -> std::pair<std::vector<DrawItem>, RenderData> {
    ...
    for (auto it = q.begin(); it != q.end(); ++it) {
        ...
        if (scroll_opt.ptr) data.scrolls[id] = scroll_opt.ptr;
        if (children_opt.ptr) {
            ...
            if (ps.clip_children || ps.overflow_clip) {
                ...
                if (first) {
                    ...
                    scissor = intersect_rectangles(static_cast<int>(scissor.x), static_cast<int>(scissor.y), 
                                                 static_cast<int>(scissor.width), static_cast<int>(scissor.height), 
                                                 cx, cy, cw, ch);
                }
 ...
 */
static void render_backgrounds(const std::vector<DrawItem>& items, const RenderData& data, 
                              r::ecs::Res<r::UiInputState> input, r::ecs::Res<r::UiTheme> theme) {
    for (const auto &it : items) {
        ...
        if (applied_scissor) {
            BeginScissorMode(static_cast<int>(scissor.x), static_cast<int>(scissor.y), 
                           static_cast<int>(scissor.width), static_cast<int>(scissor.height));
        }
        ...
        DrawRectangle(static_cast<int>(static_cast<float>(x) + sx), 
                     static_cast<int>(static_cast<float>(y) + sy), w, h, 
                     {bg.r, bg.g, bg.b, bg.a});
        
        if (border_thickness > 0.f) {
            ::Rectangle rec{ static_cast<float>(x) + sx, static_cast<float>(y) + sy, 
                           static_cast<float>(w), static_cast<float>(h) };
            DrawRectangleLinesEx(rec, static_cast<float>(border_thickness), 
...
static void render_debug_overlay(r::ecs::Res<UiPluginConfig> cfg, QueryType& q, r::ecs::Res<r::UiInputState> input) {
    ...
    if (cfg.ptr->debug_draw_bounds) {
        for (auto it = q.begin(); it != q.end(); ++it) {
            auto [node, layout, style_opt, vis_opt, parent_opt, text_opt, image_opt, button_opt, scroll_opt, children_opt] = *it;
            (void)node; (void)style_opt; (void)vis_opt; (void)text_opt; (void)image_opt; (void)button_opt; (void)scroll_opt; (void)parent_opt; (void)children_opt;
        ...
        auto draw_highlight = [&](r::ecs::Entity h, ::Color c){
            if (h == r::ecs::NULL_ENTITY) return;
            for (auto it = q.begin(); it != q.end(); ++it) {
                auto [node, layout, sopt, vopt, popt, topt, iopt, bopt, scopt, chopt] = *it;
                (void)node; (void)sopt; (void)vopt; (void)popt; (void)topt; (void)iopt; (void)bopt; (void)scopt; (void)chopt;
            ...
            if (ps.clip_children || ps.overflow_clip) {
                ...
                if (first) {
                    ...
                    sc_rect = intersect_rectangles(static_cast<int>(sc_rect.x), static_cast<int>(sc_rect.y), 
                                                 static_cast<int>(sc_rect.width), static_cast<int>(sc_rect.height), 
                                                 scx, scy, scw, sch);
                }
                sc_apply = true;
            }
...
static void render_image(r::ecs::Optional<r::UiImage> image_opt, int cx, int cy, int cw, int ch, 
                        float sx, float sy, r::ecs::ResMut<r::UiTextures> textures) {
    if (!image_opt.ptr || image_opt.ptr->path.empty()) return;
    ...
    if (tex && tex->id != 0) {
        ...
        DrawTexturePro(*tex, src, dst, {0,0}, 0.f, { image_opt.ptr->tint.r, image_opt.ptr->tint.g, image_opt.ptr->tint.b, image_opt.ptr->tint.a });
    }
}
...
static auto wrap_text(const std::string& text, ::Font font, int font_size, float spacing, float wrap_width, int content_width) -> std::string {
    if (wrap_width <= 0.f || content_width <= 0) return text;
...
static void render_text(r::ecs::Optional<r::UiText> text_opt, r::ecs::Optional<r::UiButton> button_opt, 
                       const r::Style& s, int cx, int cy, int cw, int ch, float sx, float sy,
                       r::ecs::ResMut<r::UiFonts> fonts, r::ecs::Res<r::UiTheme> theme) {
    if (!text_opt.ptr || text_opt.ptr->content.empty()) return;
...
static void render_foreground(QueryType& q, const RenderData& data, r::ecs::ResMut<r::UiTextures> textures,
    ...
    for (auto it = q.begin(); it != q.end(); ++it) {
        ...
        if (sc_apply) {
            BeginScissorMode(static_cast<int>(sc_rect.x), static_cast<int>(sc_rect.y), 
                           static_cast<int>(sc_rect.width), static_cast<int>(sc_rect.height));
        }
...
void render_system(r::ecs::Res<UiPluginConfig> cfg, r::ecs::Res<r::Camera3d> cam,
    ...
    r::ecs::Query<r::ecs::Ref<r::UiNode>, r::ecs::Ref<r::ComputedLayout>, r::ecs::Optional<r::Style>, r::ecs::Optional<r::Visibility>, r::ecs::Optional<r::ecs::Parent>, r::ecs::Optional<r::UiText>, r::ecs::Optional<r::UiImage>, r::ecs::Optional<r::UiButton>, r::ecs::Optional<r::UiScroll>, r::ecs::Optional<r::ecs::Children>> q) noexcept
{
    EndMode3D();
