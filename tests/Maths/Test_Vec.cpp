#include "../Test.hpp"

#include <R-Engine/Maths/Vec.hpp>

Test(Vec, default_constructor)
{
    r::Vec3f v;

    for (usize i = 0; i < v.size; ++i) {
        cr_expect_eq(v.data[i], 0.0f, "Default constructor should init to 0");
    }
}

Test(Vec, value_constructor)
{
    constexpr r::Vec3f v(5.0f);

    for (usize i = 0; i < v.size; ++i) {
        cr_expect_eq(v.data[i], 5.0f, "Value constructor failed");
    }
}

Test(Vec, initializer_list)
{
    constexpr r::Vec3i v{1, 2, 3};
    cr_expect_eq(v.x, 1);
    cr_expect_eq(v.y, 2);
    cr_expect_eq(v.z, 3);
}

Test(Vec, addition_operator)
{
    constexpr r::Vec3i a{1, 2, 3};
    constexpr r::Vec3i b{4, 5, 6};
    constexpr r::Vec3i c = a + b;

    cr_expect_eq(c.x, 5);
    cr_expect_eq(c.y, 7);
    cr_expect_eq(c.z, 9);
}

Test(Vec, scalar_multiplication)
{
    constexpr r::Vec2f v{1.f, -2.f};
    constexpr r::Vec2f w = v * 3.f;

    cr_expect_float_eq(w.x, 3.f, 1e-6);
    cr_expect_float_eq(w.y, -6.f, 1e-6);
}

Test(Vec, dot_product)
{
    constexpr r::Vec3i a{1, 2, 3};
    constexpr r::Vec3i b{4, 5, 6};
    int dot = a.dot(b);

    cr_expect_eq(dot, 32, "Dot product incorrect (expected 32, got %d)", dot);
}

Test(Vec, cross_product)
{
    constexpr r::Vec3i a{1, 0, 0};
    constexpr r::Vec3i b{0, 1, 0};
    const r::Vec3i c = a.cross(b);

    cr_expect_eq(c.x, 0);
    cr_expect_eq(c.y, 0);
    cr_expect_eq(c.z, 1);
}

Test(Vec, normalize)
{
    constexpr r::Vec2f v{3.f, 4.f};
    const r::Vec2f n = v.normalize();

    cr_expect_float_eq(n.length(), 1.f, 1e-6, "Normalized vector should have length 1");
}

Test(Vec, equality_operator)
{
    constexpr r::Vec2i a{1, 2};
    constexpr r::Vec2i b{1, 2};
    constexpr r::Vec2i c{2, 3};

    cr_expect(a == b);
    cr_expect(a != c);
}
