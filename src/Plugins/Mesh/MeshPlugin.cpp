...
using MeshRenderQuery = r::ecs::Query<r::ecs::Ref<r::Mesh3d>, r::ecs::Ref<r::GlobalTransform3d>, r::ecs::Optional<r::Material3d>>;

/**
 * Helpers
 */

static inline const r::Transform3d mesh_plugin_get_transform3d(const r::GlobalTransform3d &global_transform,
    const r::Mesh3d &mesh_component) noexcept
{
    const r::Quaternion logical_rot = r::Quaternion::from_euler(global_transform.rotation);

    return {.position = global_transform.position + logical_rot.rotate(mesh_component.position_offset * global_transform.scale),
...
static inline ::Shader mesh_plugin_apply_shader(::Model *model, const r::ecs::Optional<r::Material3d> &material_opt,
    r::ecs::Res<r::Shaders> &shaders) noexcept
{
    ::Shader original_shader = {};

    if (material_opt.ptr && model && model->materialCount > 0) {
        const ::Shader *custom_shader = shaders.ptr->get(material_opt.ptr->shader_id);
        if (custom_shader) {
...
static void mesh_render_system(MeshRenderQuery query, r::ecs::ResMut<r::Meshes> meshes, r::ecs::Res<r::Shaders> shaders) noexcept
{
