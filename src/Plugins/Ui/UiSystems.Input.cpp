...
static auto collect_input_items(QueryType& q) -> std::pair<std::vector<InputItem>, InputData> {
    ...
    for (auto it = q.begin(); it != q.end(); ++it) {
        ...
        if (button_opt.ptr) data.button_enabled[id] = !disabled;
...
static auto calculate_input_scroll_offset(const InputData& data, r::ecs::Entity e) -> std::pair<float, float> {
    ...
    while (p != r::ecs::NULL_ENTITY) {
        ...
        if (sit != data.styles.end() && (sit->second.clip_children || sit->second.overflow_clip) && 
            scit != data.scrolls.end() && scit->second) {
            sx -= scit->second->x;
            sy -= scit->second->y;
        }
    }
...
static auto is_inside_with_clipping(const InputItem& item, const InputData& data, float mx, float my) -> bool {
    if (!point_in_rectangle(mx, my, item.layout)) return false;
...
void pointer_system(
    ...
    r::ecs::Query<r::ecs::Ref<r::UiNode>, r::ecs::Ref<r::ComputedLayout>, r::ecs::Optional<r::Style>, r::ecs::Optional<r::Visibility>, r::ecs::Optional<r::ecs::Parent>, r::ecs::Optional<r::UiButton>, r::ecs::Optional<r::UiScroll>, r::ecs::Optional<r::ecs::Children>> q) noexcept
{
    ...
    r::ecs::Query<r::ecs::Optional<r::UiButton>, r::ecs::Optional<r::Visibility>> q)
{
    ...
    if (prev != r::ecs::NULL_ENTITY && prev != state.ptr->focused) events.ptr->blurred.push_back(prev);
